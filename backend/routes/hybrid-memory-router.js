...
const memPersist = require('../services/memory_persistence');
...
function readJSON(file, fallback) { return memPersist.readJSON(file, fallback); }
function readJsonl(file, limit = 100) { return memPersist.readJsonl(file, limit); }
function appendJsonl(file, item) { return memPersist.appendJsonl(file, item); }

function nowIso() {
  return new Date().toISOString();
}

function hashId(input) { return memPersist.hashId(input); }
function tokens(text) { return memPersist.tokens(text); }
function scoreText(query, text) { return memPersist.scoreText(query, text); }

function safeSlice(value, n = 1200) {
  return String(value || '').replace(/\s+/g, ' ').trim().slice(0, n);
}

function stateFile(...parts) { return memPersist.stateFile(...parts); }
function repoFile(...parts) { return memPersist.repoFile(...parts); }
...
function validateReadOnlySql(sql) { return memPersist.validateReadOnlySql(sql); }
function runReadOnlySql(args, actor = 'operator') { return memPersist.runReadOnlySql(args, actor); }
