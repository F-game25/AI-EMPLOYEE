...
async function main() {
  ...
  const runner = createTurnRunner({
    ...
    createWorkflowRun({ goal }) {
      return { run_id: 'run-test', goal, decisions: [] };
    },
    appendDecision(_run, entry) {
      decisions.push(entry);
    },
    attachWorkflowNode(node) {
      nodes.push(node);
    },
    addActivity(message, kind) {
      activities.push({ message, kind });
    },
    ...
    compactMemoryTraceForModel(trace) {
      return { trace_id: trace.trace_id, context: trace.context };
    },
    ...
    async isPythonBackendUp() {
      return false;
    },
    async requestPythonJSON() {
      throw new Error('not expected');
    },
    async requestPythonChatPayload() {
      throw new Error('not expected');
    },
    async requestOllamaChat() {
      return null;
    },
  ...
  assert.equal(turn.user_id, 'user:test');
  assert.equal(turn.tenant_id, 'tenant:test');
  assert.equal(turn.status, 'completed');
  assert.equal(turn.source, 'execution-engine');
  assert.ok(turn.reply.includes('I understood: build a landing page'));
  assert.ok(turn.reply.includes('Proof:'));
  assert.ok(turn.actions.some(action => action.action === 'website_builder'));
  assert.ok(turn.proof.some(item => item.label === 'index.html'));
  assert.ok(turn.artifacts.some(item => item.name === 'index.html'));
  assert.ok(events.some(item => item.event === 'turn:started'));
  assert.ok(events.some(item => item.event === 'proof:ready'));
  assert.ok(events.some(item => item.event === 'turn:completed'));
  assert.ok(events.some(item => item.event === 'orchestrator:message'));
...
main()
  .then(() => {
    console.log('PASS turn runner canonical contract');
  })
  .catch((error) => {
    console.error(error);
    process.exit(1);
