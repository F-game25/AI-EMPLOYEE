...
- `runtime/skills/` — `catalog.py`, `library.py`, definitions. The library has 570 skills backed by `runtime/config/skills_library.json` (200 curated + 370 generated by `scripts/backfill_agent_skills.py` to back every agent-advertised skill). `catalog.py` loads them by `id` into dispatchable skills.
