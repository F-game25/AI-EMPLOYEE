...
export default defineConfig({
  ...
  server: {
    ...
    // the Node backend that owns port 8787. The proxy below forwards all
    // backend paths (API, WebSocket, and static assets served by Express) to
    // the backend so relative-URL fetch calls work transparently in dev mode.
    host: '127.0.0.1',
    port: 5173,
    proxy: {
