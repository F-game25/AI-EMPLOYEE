...
# install.sh is the single installer entry point for all platforms.
# On macOS we run the macOS-specific install flow (Homebrew, Xcode CLT, shasum,
# BSD-stat paths); on Linux we fall through to the standard installer below.
# The macOS flow is embedded verbatim at the end of this file (see __MACOS_INSTALLER__)
# so there is exactly ONE installer file to maintain and ship.
if [[ "$(uname -s)" == "Darwin" ]]; then
  __mac_tmp="$(mktemp -t ai-employee-install-mac.XXXXXX.sh)"
  trap 'rm -f "$__mac_tmp"' EXIT
  # Extract everything after the __MACOS_INSTALLER__ marker into a runnable script.
  awk 'f{print} /^# __MACOS_INSTALLER__$/{f=1}' "${BASH_SOURCE[0]}" > "$__mac_tmp"
  exec bash "$__mac_tmp" "$@"
fi
# ─────────────────────────────────────────────────────────────────────────────
...
install_runtime() {
    ...
    # ── Record install baseline for updater traceability ───────────────────────
    mkdir -p "$AI_HOME/state"
    local _commit_sha=""
    if command -v git >/dev/null 2>&1 && [[ -d "$SCRIPT_DIR/.git" ]]; then
        _commit_sha="$(git -C "$SCRIPT_DIR" rev-parse HEAD 2>/dev/null || true)"
    ...
    if [[ -n "${_commit_sha:-}" ]]; then
        echo "$_commit_sha" > "$AI_HOME/state/installed_commit.txt"
        _write_version_state "$_commit_sha" "install-mac.sh"
        ok "Recorded install baseline: ${_commit_sha:0:8}"
    else
        warn "Could not record install commit SHA (offline install?) — auto-updater will bootstrap on first run"
    fi
...
CFG_END
    ...
    # Keep a symlink at the legacy path for backward compatibility.
    mkdir -p "$HOME/.ai-employee"
...
done_message() {
    ...
    echo ""
    echo "  2. Optional — link WhatsApp for notifications & quick commands:"
    echo "     Open the AI Employee dashboard and configure the WhatsApp channel"
    echo "     from Settings > Channels."
    echo ""
