...
- [Complete skills catalog (571)](#complete-skills-catalog) · [Complete agent catalog (113)](#complete-agent-catalog)
...
### 3) Skills system
A **skill** is a reusable, versioned, domain-specific workflow that composes tools into a complete capability with validated output. [`runtime/config/skills_library.json`](runtime/config/skills_library.json) ships **571 skills** across **25 categories**:
- **200 hand-curated** (incl. a native fork-enrichment layer for engineering, finance, money, autonomy, wallet, and channel work),
- **370 generated** to back **every** capability the agent catalog advertises (so no agent references a skill that doesn't exist), regenerated idempotently by [`scripts/backfill_agent_skills.py`](scripts/backfill_agent_skills.py),
- **1 vendored** multi-source research skill (`last30days`).
...
👉 **[Complete skills catalog — all 571 by category](#complete-skills-catalog)**
...
571 dispatchable skills across 25 categories. Each is registered from
[runtime/config/skills_library.json](runtime/config/skills_library.json) and runs via the
agent controller / companion `skills.run` path. Expand a category to see every skill.
