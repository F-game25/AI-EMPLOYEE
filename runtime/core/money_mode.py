...
class MoneyMode:
    ...
    def run_content_pipeline(
        ...
        *,
        ...
        # Step 2 — draft REAL content per platform via content_publish_track
        # (generates with the LLM + saves a real .md artifact; falls back to a
        # clearly-labeled template only when the LLM is offline). This replaces
        # the previous canned per-platform template strings.
        artifacts: list[dict] = []
        real_drafts = 0
        for platform in platforms:
            art = self.content_publish_track(topic, platform=platform, content_type="post")
            artifacts.append(art)
            if art.get("status") == "draft":
                real_drafts += 1
            steps.append({
                "step": "draft_content", "platform": platform,
                "output": {"word_count": art.get("word_count"),
                           "file_path": art.get("artifact"),
                           "content_status": art.get("status")},
                "status": "done", "requires_review": True,
            })
        ...
        # Engagement is an unvalidated PROJECTION, not a measured result — labelled
        # as such so it is never shown as a real outcome.
        engagement_estimate = round(10 * max(len(platforms), 1) * _ENGAGEMENT_FACTOR, 3)
        steps.append({
            "step": "track_engagement",
            "output": {
                "projected_interactions": engagement_estimate,
                "is_projection": True,
        ...
        return {
            ...
            "affiliate_product": affiliate_product,
            "steps": steps,
            "artifacts": artifacts,           # real saved content files + word counts
            "real_drafts": real_drafts,       # how many were LLM-generated (not template)
            "estimated_roi": estimated_roi,
            "engagement_score": engagement_estimate,
            "estimates": True,                # roi/engagement are projections, not measured
            "status": status,
            "note": (
                f"{real_drafts}/{len(platforms)} drafts generated by the LLM and saved as artifacts. "
                "ROI/engagement are projections. Posting requires manual approval."
            ),
        }
    ...
    def run_lead_pipeline(
        ...
        *,
        ...
        job_id = str(uuid.uuid4())[:8]
        # REAL scrape when the source is a URL — fetch+extract+store via
        # data_scrape_filter_store. No URL → honest 0 records (no fabrication).
        is_url = source.strip().lower().startswith(("http://", "https://"))
        if is_url and not dry_run:
            scrape = self.data_scrape_filter_store(source.strip(), topic=audience)
            words = int(scrape.get("words_extracted") or 0)
            stored = bool(scrape.get("stored"))
            # qualified leads is a PROJECTION derived from real extracted volume
            filtered_leads = max(words // 200, 1) if words else 0
            scrape_step = {
                "step": "scrape_data",
                "input": {"source": source, "audience": audience},
                "output": {
                    "words_extracted": words, "stored": stored,
                    "duplicate": bool(scrape.get("duplicate")),
                    "signal_source": source, "ingestion_channels": ingestion_channels,
                },
                "status": "done" if scrape.get("ok") else "failed",
                "error": scrape.get("error"),
            }
            scraped_records = words
        else:
            filtered_leads = 0
            scraped_records = 0
            scrape_step = {
                "step": "scrape_data",
                "input": {"source": source, "audience": audience},
                "output": {"records_scraped": 0, "ingestion_channels": ingestion_channels},
                "status": "dry_run" if dry_run else "skipped",
                "note": "Provide a URL source to scrape real data" if not is_url else "dry run",
            }
        steps: list[dict] = [
            scrape_step,
            {
        ...
        return {
            ...
            "steps": steps,
            "estimated_roi": conversion_estimate,
            "estimates": True,   # roi/qualified_leads are projections from real scrape volume
            "status": status,
        }
    ...
    def run_opportunity_pipeline(
        ...
        *,
        ...
        return {
            ...
            "steps": steps,
            "estimated_roi": estimated_roi,
            "estimates": True,   # contacts/responses/conversions/roi are forecasts for an unsent campaign
            "status": status,
