...
def ai_query(prompt: str, system_prompt: str = "") -> str:
    ...
    try:
        result = _query_ai_for_agent("referral-rocket", prompt, system_prompt=system_prompt)
...
def handle_command(message: str) -> str | None:
    ...
    if rest_lower.startswith("incentive "):
        ...
        budget = parts[1] if len(parts) > 1 else DEFAULT_REFERRAL_BUDGET
