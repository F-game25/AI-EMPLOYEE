...
2. **B2 convert batch 1** — **DONE (2026-06-30).** 12 revenue/lead/content/research skills: `market_research, competitor_analysis, sentiment_analysis, lead_scraping, blog_writing, content_calendar, industry_report, swot_analysis, pricing_analysis, keyword_research, trend_analysis_brief_builder, product_research` → `web_search → llm_infer`. `tests/test_skill_chain_batch1.py` 15/15. **B2 convert batch 2** — **DONE (2026-07-01).** 15 strategy/analysis/content skills across two patterns — **Pattern A** (research+synthesis, 8 skills: `business_plan_generation, go_to_market, ab_test_analysis, funnel_analysis, cohort_analysis, forecasting, whitepaper_writing, case_study_writing`) → `web_search → llm_infer`; **Pattern B** (pure synthesis, 7 skills: `email_copywriting, ad_copywriting, press_releases, social_captions, youtube_scripts, newsletter_writing, headline_generation`) → `llm_infer` only. Conversion scripted + idempotent (`scripts/convert_batch2_skill_chains.py`). `tests/test_skill_chain_batch2.py` 34/34 (chain shape, pattern A/B contract, risk≤1, end-to-end threading, output count). **Total executable skills: 27/570.** C2 proof suite 194/194.
