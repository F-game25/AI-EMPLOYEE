...
> **Headline (updated 2026-06-30):** C2's *infrastructure* (B3 + the tool-chain interpreter) is done **and the payload is now flowing** — 12 batch-1 revenue/lead/content/research skills declare real `tool_steps` and run a deterministic `web_search → llm_infer` chain through the `ToolRegistry` (proof in §4 step 2). Remaining for C2: **convert further batches** of skills, and **B1** — route `BaseAgent.execute` (still a direct LLM call) through the same skill chain, revenue/lead agents first.
...
2. **B2 convert batch 1** — **DONE (2026-06-30).** 12 revenue/lead/content/research skills now carry an executable `tool_steps` chain (`web_search → llm_infer`, both risk-0 → auto-run, no HITL): `market_research, competitor_analysis, sentiment_analysis, lead_scraping, blog_writing, content_calendar, industry_report, swot_analysis, pricing_analysis, keyword_research, trend_analysis_brief_builder, product_research`. The `llm_infer` synthesis prompt is **derived from each skill's own `description` + `execution_steps`** (no per-skill hardcoded instructions) and labels web findings as **UNTRUSTED reference DATA** (prompt-injection guard). Conversion is scripted + idempotent (`scripts/convert_batch1_skill_chains.py`); 570-skill total preserved; library encoding (escaped non-ASCII) kept so the diff is only the additions. `tests/test_skill_chain_batch1.py` 15/15 (chain shape, risk≤1, real ordered execution, default-ceiling auto-run); existing `test_skill_tool_chain.py` 9/9 + `test_skill_chain.py` 5/5 green; 190 skill/dispatch tests pass; lint clean. **A converted skill now returns `via:"skill_tool_chain"` with two real per-tool results — not a single LLM blob.**
3. **B1 batch-1 — `_emit_action` routes through skill catalog** — **DONE (2026-07-01).** `AgentController._emit_action` (`core/agent_controller.py`) now dispatches through `ExecutableSkillCatalog.dispatch_for_goal(goal, ctx={"skill_id": skill})` before falling to a bare role-prompt. Routing chain: tool-chain (if skill has `tool_steps`) → library-guided LLM (skill's own `system_prompt`/`execution_steps`) → bare role-prompt (floor, no skill entry). Fail-closed: `error`/`blocked` from the chain propagate as `RuntimeError` rather than silently falling back. `tests/test_agent_controller_b1.py` 6/6. *(next increment: convert further skill batches, then B1 for individual BaseAgent subclasses)*
