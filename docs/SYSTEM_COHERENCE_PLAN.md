...
### Phase C1 — One execution spine (P0: A1,A2 · P1: A3,A4)  — **STATUS: DONE (2026-06-30) — see `docs/SYSTEM_COHERENCE_C1_PLAN.md`**
> Earlier-merged: A1-Python single entry (`12e5fd1d`), A3 intent seam (`3717a508`), companion→skill-chain (`87ba23ea`), B3 real agent activity (`ff5ba02d`), G5 GoalRegistry (`032c6c11`). Remaining slice shipped 2026-06-30 (order R2→R4→R1→R3): **R2** companion sources intent through the seam (`95bb0cf6`, perf-safe opt-out of the LLM axis); **R4** registry-backed agent routing, static `_INTENT_AGENT_PROFILES` retired to score-profile/fallback (`98fe93bf`); **R1** chat goes pipeline-first, redundant `run_execution.py` rung skipped, `TURN_RUNNER_PIPELINE_FIRST` defaults ON, legacy WS ladder inverted too (`1864fb63`); **R3** broker side-effect audit + invariant lock-in, no violations (`c91b8d5a`). 151 Python + Node turn-runner + lint green. Full detail in **`docs/SYSTEM_COHERENCE_C1_PLAN.md`**.
...
### Phase C2 — One agent/skill/tool chain (P0: B1,B2,B3)  — **STATUS: B3 DONE · B2 interpreter + batch-1 (12 skills) + batch-2 (15 skills, 27 total, 2 patterns) DONE · B1 `_emit_action` gateway DONE · further skill batches + per-agent BaseAgent migration remaining — full detail in `docs/SYSTEM_COHERENCE_C2_PLAN.md`**
> Reconciliation (2026-06-30): **B3 done** (`ff5ba02d`). **B2 interpreter done** (`de34f5c5` — `_run_tool_chain`/`_resolve_inputs`, risk-gated, fail-closed). **B2 batch-1 done** — 12 revenue/lead/content/research skills now declare executable `tool_steps` (`web_search → llm_infer`, risk-0 auto-run) derived from each skill's own contract; they return `via:"skill_tool_chain"` with real per-tool results, not an LLM blob (`scripts/convert_batch1_skill_chains.py`, `tests/test_skill_chain_batch1.py` 15/15, 570-skill total preserved). **Remaining:** convert further skill batches; **B1** — route `BaseAgent.execute` through the skill chain (revenue/lead agents first). Lives in **`docs/SYSTEM_COHERENCE_C2_PLAN.md`**.
...
### R-5 — AscendForge Goal Achievement System (TQ series)  (added 2026-07-01, status: TRIAGED — plan approved, execution starting)
- **Source:** Lars — "the task queue needs to be fully built" → corrected to "this is not a task system, it's a goal-achieving system": give it a goal, it plans, it puts agents on the sub-tasks, must also handle short tasks, everything UI-relevant must be tracked for the later UI pass. Full plan: `ASCENDFORGE_SYSTEM_AUDIT.md` Phase E.
- **Summary:** AscendForge already has three overlapping, unconsolidated execution paths (`forge_queue_item`+dispatcher; Backlog+Autopilot+Decomposer+Cycles; the basic `/runs` single-shot path). A real goal-container (`forge_cycles`) and a real sub-task/child-run schema (`forge_child_runs`) already exist in `backend/services/forge_store.js` but are incomplete/unused — Cycles never learn their own goal completed; child-runs has zero call sites. The Decomposer already asks the LLM for `required_skills` per subtask and throws the answer away — nothing routes a subtask to the right skill/agent, every subtask runs the same generic coder/tester loop. Two live bugs found and fixed this session in the shared codegen extractor (`extractCodeActions`) also benefit this path. Measured: local model (qwythos:q4, kept per Lars) succeeds ~60-70% single-shot on a trivial task; 4 concurrent forge runs degrade badly (2-of-4 Ollama calls failed) — an 8GB-VRAM hardware constraint, not just a software gap.
- **Maps to:** Theme B (agents/skills/tools disconnection) — the agent/skill-routing piece (TQ-2) is an AscendForge-scoped instance of Phase C2's "agents dispatch through skills, not bespoke LLM calls." Also touches Theme F (observability — new WS events) at the AscendForge UI layer.
- **Reality check:** confirmed via direct code reading, not assumed — `forgeRunStore` is already real SQLite (`forge_backlog`, `forge_cycles`, `forge_child_runs` tables), not JSON-only; `runtime/forge/lifecycle/skill_selector.py::select_skills()` (keyword/tag scoring against the real 570-skill library) already exists and is unused for decomposer routing; `runtime/agents/business_swarm/{task_decomposer,assignment_engine,dependency_manager,parallel_executor,result_aggregator}.py` (Module 7 of `MASTER_PLAN_V3.md`, confirmed **fully built** — all 10 files present, cross-verified independently and via PR #335's gap audit) already implements the same decompose→assign→parallel-execute→aggregate shape, currently wired only into `runtime/companyos/company_planner.py` (business domain) — TQ-2 must evaluate reusing/generalizing this pattern before writing new decomposition/assignment logic in `forge.js`.
- **Action:** TQ-1 canonical queue consolidation (durable autopilot state, finish the Cycle lifecycle) → TQ-2 fast-path + skill/agent routing → TQ-3 execution capability uplift (file tools, iterative verify, sub-task delegation via `forge_child_runs`, branching) → TQ-4 concurrency gate + retry tuning → TQ-5 UI/observability (Cycles/Backlog/Autopilot currently have zero UI surface) → TQ-6 safety hardening cross-cutting. Full detail, file:line citations, and verification plan per phase in `ASCENDFORGE_SYSTEM_AUDIT.md` Phase E.
- **Decision needed:** none blocking — adapter-mode queue cutover, autonomy-ceiling default, and approvals-UI consolidation already confirmed by Lars (see Phase E). Open: whether to reuse `business_swarm`'s modules directly vs. just its pattern (verify during TQ-2 implementation); whether to merge PR #335 (`claude/gap-analysis-deep-dive-twiozu`, unmerged — file-level gap audit of `MASTER_PLAN_V3.md`, independently cross-confirms the business_swarm/skill_selector findings above).
